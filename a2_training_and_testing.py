...
def train_for_epoch(model, dataloader, optimizer, device):
    ...
    for data in tqdm(dataloader):
        # 1. Send to appropriate device
        F, F_lens, E = data
        F = F.to(device)
        F_lens = F_lens.to(device)
        E = E.to(device)
        # 2. Zeros out the model's previous gradient
        optimizer.zero_grad()
        # 3. Determine next-token probabilities.
        logits = model(F, F_lens, E)
        # 4. Modifies ``E`` for the loss function
        pad_mask = model.get_target_padding_mask(E)
        E = E.masked_fill(pad_mask, -1)
        # 5. Flattens out the sequence 
        logits = logits.flatten(end_dim=1)
        E = torch.flatten(E[1:], start_dim=0)
        # 6. Calculate the batch loss
    ...
    avg_loss = total_loss / total_lens
    return avg_loss
...
def compute_average_bleu_over_dataset(
    ...
    with torch.no_grad(): 
        for F, F_lens, E in tqdm(dataloader):
            ...
            n_sequences += F_lens.shape[0]
            del F, F_lens, E
    torch.cuda.empty_cache()
    avg_bleu = total / n_sequences
    return avg_bleu
